...
function pollSession(sessionUri) {
  debug('Polling flight search session for data: ' + sessionUri + ' with params ' + JSON.stringify(this.filter));

  var scanner = this;

  var uri = sessionUri,
      params = Object.keys(this.filter),
      i,
      len = params.length,
      uriParams = [];
  for (i = 0; i < len; i+=1) {
    uriParams[i] = params[i] + '=' + this.filter[params[i]];
  }
 ...
 * @param      {object}      filter   Results filter.
 */
function Skyscanner(options, filter) {
